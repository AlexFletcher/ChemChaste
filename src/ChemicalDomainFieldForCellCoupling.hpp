...
void ChemicalDomainFieldForCellCoupling<ELEMENT_DIM,SPACE_DIM,PROBLEM_DIM>::MapToChasteCartesianCellLayer()
{
    ...
    unsigned numXSteps = std::round((mCartesianCellLayerDimensions[0]*mCartesianCellLayerScaleXY[0])/cartesianChasteCellScaleXY[0]); 
    unsigned numYSteps = std::round((mCartesianCellLayerDimensions[1]*mCartesianCellLayerScaleXY[1])/cartesianChasteCellScaleXY[1]); 

    std::vector<std::vector<std::string>> cartesianChasteCellLayer;
    std::vector<unsigned> cartesianChasteCellDimensions;
    cartesianChasteCellDimensions.push_back(numXSteps);
    cartesianChasteCellDimensions.push_back(numYSteps);
...
template<unsigned ELEMENT_DIM,unsigned SPACE_DIM,unsigned PROBLEM_DIM>
unsigned ChemicalDomainFieldForCellCoupling<ELEMENT_DIM,SPACE_DIM,PROBLEM_DIM>::GetNumCellTypes()
{
    return mNumCellTypes;
}
